...
One row of `GET /activity-logs`. Verified against the backend at `0d81988`.

| Field | Notes |
|---|---|
| `id` | the entry's own key, not the subject's |
| `type` | the `ActivityLogAction` enum **value**, already a display string ("Transaction - Void"), not a code |
| `context` | the readable past-tense sentence, generated by the formatter when the entry was written |
| `created_at` | full ISO-8601 timestamp. `ActivityLog` sets `$timestamps = false`, so there is no `updated_at` |
| `actor` | `{id, name, role}` |

**The readable sentence is `context`, not `description`.** Issue #63 calls
it a description throughout; the wire does not.

**The actor is always renderable.** A system-generated entry (`actor_id`
null) arrives as `{id: null, name: "System", role: null}`, so there is no
null case for the UI to write copy for. The relation is eager-loaded by
the controller, so the `whenLoaded` guard never omits it in practice.

There is **no `subject` and no `details` on a list row.** Both are why the
detail request exists.

### `ActivityLogResource`

`GET /activity-logs/{activity}`. A superset of the list row, plus:

| Field | Notes |
|---|---|
| `subject` | `{type, id, exists}`. `type` is `Str::snake(class_basename(subject_type))`, so `transaction`, `service`, `series_receipt`, `user` |
| `details` | `array<{label, value}>`, from the action's registered formatter |

**`details` values are fully formatted server-side** — peso signs and
`from → to` arrows included. The formatter interface requires the
label/value shape for every action, which is what lets all seventeen event
types render through one client component with no per-type branching. Do
not parse or re-format them.

**An account's subject `type` is `user`, not `account`.** `ACCOUNT_CREATED`
logs against the `User` model.

**Three of the seventeen actions have no registered formatter**, and
`ActivityLogFormatterRegistry::resolve` throws rather than degrading — so
the *detail* endpoint 500s on one, while the list row renders fine.
The three are `TRANSACTION_GENERATE_RECEIPT`, `SERVICE_ACTIVATED` and
`SERVICE_DEACTIVATED`. Nothing writes them today, so no such rows exist.
If the backend starts logging service activation, the drawer breaks until
a formatter is registered. Worth flagging there rather than defending
against in the client.

**The two resources format the actor differently.** The list builds the
name from `full_name` and reads `roles->first()`; the detail concatenates
`first_name` and `last_name` and calls `getRoleNames()`. They agree today.
