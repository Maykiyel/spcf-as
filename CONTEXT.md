...
The page at `/transactions/receipts`, and the app's only route to *finding* a transaction. A server-paginated, server-sorted, server-filtered table over `GET /transactions`; a row opens the existing View Transaction page for that control ID. Shared by both roles, with the endpoint doing the scoping: an admin sees every transaction, a cashier sees only their own, and nothing in the frontend re-implements that rule. **It has no search box** � `/transactions` accepts no `filter[search]`, and an unknown filter key is a 400 here rather than an ignored parameter. It narrows by payer name, series number, item name, status and date range, plus cashier for an admin; the cashier filter is gated by being *declared* only for an admin, not merely by its control being hidden, because a declared filter is what the URL can restore. Rows show item names as a summary and no per-item money: the list endpoint returns items stripped to an id and a name, and per-item money is what the detail page is for.
_Avoid_: transactions list (ambiguous with the Itemized List page and with the Void page, which is the same table with status pinned), receipts page (a Series receipt is a different thing entirely � see above)
